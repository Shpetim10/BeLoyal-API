...
public class AdminBusinessRegistrationController {
    ...
    public ResponseEntity<Map<String,String>> approveBusiness(@PathVariable("businessId") Long businessId, @AuthenticationPrincipal UserPrincipal userPrincipal){
        try{
    ...
    @PreAuthorize("hasRole('SUPER_ADMIN')")
    public ResponseEntity<Map<String,String>> rejectBusiness(@PathVariable("businessId") Long businessId, @RequestBody String rejectionReason, @AuthenticationPrincipal UserPrincipal userPrincipal){
        try{
            if(userPrincipal == null){
                return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Map.of(HEADER_KEY, "Access denied"));
            }

            adminBusinessRejectionService.rejectBusinessRegistration(businessId, userPrincipal.getId(), rejectionReason);
